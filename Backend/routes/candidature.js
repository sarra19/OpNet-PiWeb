...
router.post("/add", upload.fields([
  ...
  try {
    const { nom, email, specialite, offreId } = req.body;

    // Vérifier si tous les champs sont renseignés
    if (!nom || !email || !specialite || !req.files["lettreMotivation"] || !req.files["cv"] || !offreId) {
      return res.status(400).send("Tous les champs sont obligatoires.");
    }

    // Ajouter la candidature à la base de données avec la référence à l'offre
    const candidature = await Candidature.create({ nom, email, specialite, lettreMotivation: req.files["lettreMotivation"][0].originalname, cv: req.files["cv"][0].originalname, offre: offreId });
