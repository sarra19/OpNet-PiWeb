...
const Quiz = require("../models/quiz"); // Modifier l'importation du modèle Quiz
...
// Route pour récupérer le quiz associé à une offre spécifique et à un ID de quiz spécifique
router.get('/quiz/:offerId', async (req, res) => {
  const offerId = req.params.offerId;

  try {
    // Recherchez le quiz associé à l'ID de l'offre dans la base de données
    const quizInstance = await Quiz.findOne({ offreId: offerId });
    ...
    // Envoyez le quiz trouvé en tant que réponse
    res.json(quizInstance);
  } catch (error) {
    // En cas d'erreur, envoyez un code d'erreur et un message d'erreur
